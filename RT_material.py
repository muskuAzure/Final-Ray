...
class IsotropicVolume(Material):
    def __init__(self, cAlbedo, fDensity=1.0) -> None:
        """
        Initialize isotropic volume material
        cAlbedo: Color of the volume
        fDensity: Density of the volume (controls how quickly light attenuates)
        """
        super().__init__()
        self.albedo = cAlbedo
    ...
    def scattering(self, rRayIn, hHinfo):
        """
        Generate scattered ray in random direction
        For isotropic media, scattering is uniform in all directions
        """
        scattered_direction = rtu.Vec3.random_vec3_unit()
        scattered_ray = rtr.Ray(hHinfo.getP(), scattered_direction, rRayIn.getTime())
        attenuation_color = self.BRDF(rRayIn, scattered_ray, hHinfo)
        return rtu.Scatterinfo(scattered_ray, attenuation_color)
    ...
    def BRDF(self, rView, rLight, hHinfo):
        """
        Return albedo color - isotropic scattering is equal in all directions
        """
        return self.albedo
    
    def sample_distance(self):
        """
        Sample a random distance based on exponential distribution
        Returns distance where scattering/absorption occurs
        """
        return -math.log(1 - rtu.random_double()) / self.density
    
    def is_light(self):
        """
        Volume materials are not light sources
        """
        return False